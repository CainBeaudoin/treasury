...
- If a user keeps a prize in the vault, no immediate settlement is issued and no USDC leaves the treasury yet. The item becomes a pending exposure with:
  - current FMV
  - a 365-day liquidation option at `70% × min(initial prize FMV, current live FMV)` in the original funding asset
  - expected physical fulfillment cost basis
  - days remaining in the 365-day liquidation window
...
- Pending-item liquidation is available for 365 days only. It is capped on upside and follows downside: if market rises above the reveal value, the initial FMV remains the ceiling; if market falls, the lower live FMV is used. After day 365, the liquidation option expires and no automatic fallback is paid.
...
The page uses in-memory demo data and simulates new activity every 15 seconds. Replace the arrays in `app.js` with production API responses and use WebSocket/SSE events for live updates.

## Deploy

The project is dependency-free and can be imported directly into Vercel with no build command.
